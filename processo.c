...
void encontrarProcessoMaisAntigo(const Processo *p, int n)
{
    if (!n)
    {
        puts("Nenhum processo encontrado para analise.");
        return;
    }
    int idx = -1, menor = -1;
    for (int i = 0; i < n; i++)
    ...
    if (idx == -1)
        puts("Nao foi possivel determinar o mais antigo (todas as datas invalidas).");
    else
...
int contarViolenciaDomestica(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_violencia_domestica;
    return c;
}
int contarFeminicidio(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_feminicidio;
    return c;
}
int contarAmbiental(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_ambiental;
    return c;
}
int contarQuilombolas(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_quilombolas;
    return c;
}
int contarIndigenas(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_indigenas;
    return c;
}
int contarInfancia(const Processo *p, int n)
{
    int c = 0;
    for (int i = 0; i < n; i++)
        c += p[i].flag_infancia;
...
void salvarDiasResolucaoCSV(const Processo *p, int n, const char *arq)
{
...
void imprimirDiasResolucaoAmostra(const Processo *p, int n, int maxl)
{
    int m = 0;
    for (int i = 0; i < n && m < maxl; i++)
    {
        int dr = dataParaDias(p[i].dt_recebimento), dj = dataParaDias(p[i].dt_resolvido);
        if (dr != -1 && dj != -1 && dj >= dr)
        {
...
void calcularCumprimentoMeta1(const Processo *p, int n)
{
    ...
    if (den <= 0)
    {
        puts("Nao foi possivel calcular a Meta 1, denominador e zero.");
        return;
    }
...
double calcularCumprimentoMeta1Streaming(const char *arq, int ano, long long *oc, long long *oj, long long *od, long long *os)
{
    ...
    while (fgets(ln, sizeof(ln), fp))
    {
        ...
        {
            char pref[6];
            snprintf(pref, sizeof(pref), "%04d-", ano);
            if (strncmp(c[9], pref, 5) != 0)
                continue;
        }
